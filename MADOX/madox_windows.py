...
def message(typing_speed=0.002, pause_time=2):
   text1 = "This is a python tool for cloning websites and webpages for penetration testing. ;)\n"
   text2 = "This tool was developed by OPTIMUS07. https://github.com/OSBORNNARTEY/MADOX.\n"
...
use_wget = input(colored("[!!] Download Webpage with wget? (Y/n) : ", "green"))

if use_wget.lower() not in NO:
    url = input(colored("[!!] Enter FULL URL : ", "green"))

    user_agent_change = input(colored("[*] Use default user agent? (Y/n) : ", "green"))
    
    if user_agent_change.lower() not in NO:
        user_agent = DEFAULT_USER_AGENT
    else:
        user_agent = input(colored("[?] User agent : ", "green"))

    print(colored("\n[*] Downloading web page with wget ...", "blue"))
...
redirect_ip = input(colored("[?] What is the IP/domain GET/POST should forward to : ", "green"))

#########################
###  EDIT HTML FORMS  ###
#########################

print(colored("\n[*] Editing HTML index file ...", "red"))


# Yes, I do in fact parse HTML with Regex, I know.
html_as_str = open(INDEX_PATH, 'r', encoding='utf-8').read()
                                                      # Open the HTML as a string
